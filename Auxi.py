...
def down_tension( Data, l1, R ) -> list : # Estimated tensions in descent
    ...
    if condition_signal_change == "No":
        ...
        tension_change = C1*np.sin(angle-angle_variation[condition_where_change]) + C2*np.cos(angle-angle_variation[condition_where_change]) + K*( np.exp(-Data.µ*(angle-angle_variation[condition_where_change])) ) #CALCULA A TENSÃO NO PONTO QUE TROCOU O SINAL
